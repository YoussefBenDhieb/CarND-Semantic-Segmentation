...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # TODO: Implement function
    # 1x1 convolution for vgg_layer7_out
    ...
    # 1x1 convolution for vgg_layer4_out
    layer4_1x1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, padding='same',
        kernel_initializer = tf.random_normal_initializer(stddev=0.01),  kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # addition 
    layer4_add = tf.add(layer7_up, layer4_1x1)
    # upsampling of layer4_add
    layer4_up = tf.layers.conv2d_transpose(layer4_add, num_classes, 4, 2, padding='same',
        kernel_initializer = tf.random_normal_initializer(stddev=0.01), kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    
    # 1x1 convolution for vgg_layer3_out
    layer3_1x1 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding='same',
        kernel_initializer = tf.random_normal_initializer(stddev=0.01),  kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # addition 
    layer3_add = tf.add(layer4_up, layer3_1x1)
    # upsampling of layer4_add
    output_layer = tf.layers.conv2d_transpose(layer3_add, num_classes, 16, 8, padding='same',
        kernel_initializer = tf.random_normal_initializer(stddev=0.01),  kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def run():
    ...
    with tf.Session() as sess:
        ...
        logits, cross_entropy_loss, train_op = optimize(output_layer, correct_label, learning_rate, num_classes)
